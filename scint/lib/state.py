...
from typing import Generic, Type, TypeVar, List, Callable, Optional, Any, Dict

from scint.lib.schema.models import Model
...
class StateContextManager:
    def __init__(self, obj: State, state: AgentState):
        self.obj = obj
        self.state = state
    ...
    def __exit__(self, exc_type, exc_value, traceback):
        ...
        if self.state.next:
            next_state = AgentState[self.state.next]
            self.obj.transition_to(next_state)
        return False


class StateProxy:
    def __init__(self, obj: AgentState):
        self.obj = obj
...
class State:
    def __init__(self, owner):
        self.current = AgentState.STARTING
        self.history: List[AgentState] = [self.current]
